...
- **`auth.redirectPort` config**: Pin the OAuth callback to a fixed port by adding `"auth": { "redirectPort": 9999 }` to your MCP server configuration. This is useful in environments where dynamic port allocation is restricted or blocked by a firewall.
