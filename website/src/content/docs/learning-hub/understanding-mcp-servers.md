...
- **`/mcp list`**: Run `/mcp list` inside a session to see all currently attached MCP servers and their status (connected, disconnected, sandboxed, etc.). This command is available even while the agent is working, so you can inspect server status mid-turn without interrupting the agent.
