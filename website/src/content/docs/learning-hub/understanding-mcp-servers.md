...
- **`client_credentials` grant (v1.0.40+)**: For fully automated pipelines where even a device code flow is impractical, MCP servers that support the OAuth `client_credentials` grant type can authenticate without any browser interaction or secondary device. This enables true machine-to-machine authentication — the server issues a token using pre-configured client credentials, so Copilot CLI can connect to protected MCP servers in headless CI/CD environments without any human interaction.
