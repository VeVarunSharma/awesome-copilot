...
- **`client_credentials` OAuth grant (v1.0.40+)**: MCP servers that support the `client_credentials` OAuth grant type are now fully supported, enabling **completely headless authentication** with no browser redirect or device code required. This is ideal for server-to-server integrations, CI pipelines, and automation contexts where interactive authentication is not possible.
