...
- **`client_credentials` OAuth grant type (v1.0.40+)**: MCP servers can use the `client_credentials` grant type for fully headless authentication — no browser redirect required. This is ideal for CI/CD pipelines, server-to-server integrations, and automated workflows where user interaction is not possible. Servers that support this grant type will authenticate silently using their client ID and secret.
