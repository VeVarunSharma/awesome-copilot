...
- **`client_credentials` OAuth grant type**: For fully automated pipelines or servers where neither a browser nor a device is available, MCP servers can use the OAuth `client_credentials` grant type. This enables machine-to-machine authentication with no user interaction whatsoever — ideal for CI systems and background services.
...
Some MCP servers can declare **task support** — they expose tools that run as non-blocking background agents rather than synchronous tool calls. This is particularly useful for long-running operations such as builds, test suites, or data processing pipelines where you don't want the conversation to stall while waiting for completion.

When an MCP tool declares `taskSupport: "required"`, Copilot CLI automatically runs it as a background agent. You can track and manage these background tasks using built-in agent management tools:

- **`list_agents`** — list all active background agents and their status
- **`read_agent`** — read the output of a specific background agent

This lets you kick off a long-running operation and continue the conversation while it completes in the background.

> **Note**: MCP Tasks are an experimental feature. Enable experimental mode with `/experimental on` or the `--experimental` flag to unlock tools that declare `taskSupport: "required"`.
