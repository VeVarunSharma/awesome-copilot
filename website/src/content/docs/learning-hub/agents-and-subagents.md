...
- **Inherited tool restrictions**: when a custom agent launches subagents, those subagents automatically keep the parent agent's tool filters. This means the security boundaries you define in your agent—such as restricting access to certain tools or MCP servers—are enforced throughout all nested subagent calls, not just the top-level turn.
