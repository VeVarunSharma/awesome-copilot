...
| `agentStop` | Main agent finishes responding to a prompt | Run final linters/formatters, validate complete changes — see loop-prevention note below |
...
> **Loop prevention (v1.0.72+)**: An `agentStop` hook that *always* exits with a non-zero code would otherwise cause an infinite loop — the agent stops, the hook blocks, the agent retries, and so on. To prevent this, after **8 consecutive blocks** the CLI forces the turn to end and passes a `stop_hook_active: true` flag in the hook's JSON input. Check for this flag in your script to self-limit and avoid looping:
>
> ```bash
> #!/usr/bin/env bash
> INPUT=$(cat)
> # Don't re-run if CLI is already in forced-stop mode
