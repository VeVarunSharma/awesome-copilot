...
**argument-hint**: A short human-readable hint shown alongside the skill name in the slash-command picker to guide users on what to pass as input. This helps users understand what context or parameters to provide when invoking the skill.

```yaml
---
name: explain-code
description: 'Explain what a piece of code does, covering its purpose, logic, and any important caveats'
argument-hint: 'Paste the code snippet or describe the file/function to explain'
---
```
...
GitHub Copilot CLI provides a dedicated `copilot skill` subcommand (also accessible as `/skill` inside a session) to list, add, and remove skills without manually copying folders.

### Listing Skills

```bash
# List all skills currently available in the session
copilot skill list
```
...
You can add a skill from a local folder, a URL, or a directory of skills:

```bash
# Add a skill from a local folder
copilot skill add ./path/to/my-skill

# Add a skill from a URL (e.g., a GitHub raw link)
copilot skill add https://example.com/skills/my-skill

# Add all skills in a directory
copilot skill add ./path/to/skills-dir/
```
...
### Reviewing Draft Skill Changes with `/chronicle`

The `/chronicle skills review` command lets you review proposed skill changes (drafts) generated by Copilot during a session. For each proposed draft, you can:

- **Accept** — apply the draft skill to your configuration
- **Reject** — discard the draft
- **Defer** — skip for now and decide later

```
/chronicle skills review
```

This is especially useful when you've asked Copilot to help improve or scaffold skill files and want to review each suggestion before committing it.
