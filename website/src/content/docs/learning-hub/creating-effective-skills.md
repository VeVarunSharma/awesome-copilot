...
A: Skills can be invoked in several ways. Users can type the skill name as a `/command` in VS Code Chat (e.g., `/generate-tests`). In Copilot CLI and ACP clients (e.g., Zed), skills are also available as slash commands — type `/skill-name` directly in those interfaces. Agents can also discover and invoke skills automatically based on the skill's description and the user's intent.
