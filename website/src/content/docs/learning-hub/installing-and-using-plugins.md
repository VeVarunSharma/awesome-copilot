...
| **Extensions** | Native Copilot CLI extensions (e.g., custom UI panels, tool integrations) | `com.github.copilot/extensions/` |
...
To auto-update plugins from an additional marketplace at session start, add `"autoUpdate": true`:
...
> **Note**: `autoUpdate` is opt-in per marketplace. The built-in `copilot-plugins` and `awesome-copilot` marketplaces are not affected by this setting.
