...
**reasoning** (optional): Sets the reasoning effort level the agent uses when generating responses. Accepted values are `low`, `medium`, and `high`. Higher effort produces more thorough outputs at the cost of latency; lower effort is faster and suited for straightforward tasks. When omitted, the model's default reasoning level applies.
